...
public class TriggerExecutionThread implements Runnable {
	...
	protected void processLogEntry(LogEntry logEntry) throws Exception,
			Throwable {
		// Make sure its mine, or its old enough that I should pick
		// it up to ensure processing by someone
		if (CommitLog.getCommitLog().isMine(logEntry)
				|| CommitLog.getCommitLog().isOld(logEntry)) {
			...
			if (!LogEntryStatus.ERROR.equals(logEntry.getStatus())) {
				...
				String path = logEntry.getKeyspace() + ":"
						+ logEntry.getColumnFamily();
				List<Trigger> triggers = TriggerStore.getStore().getTriggers()
						.get(path);
				if (triggers != null && triggers.size() > 0
						&& triggers.get(0) instanceof PausedTrigger) {
					logger.debug("Paused triggers for: "
							+ logEntry.getColumnFamily());
				} else {
					if (triggers != null) {
